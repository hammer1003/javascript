...
function createModel() {
  ...
  model.add(
    tf.layers.conv2d({
      ...
      filters: 8,
      strides: 1,
      activation: "relu", // reactive fide linear unit
      kernelInitializer: "VarianceScaling"
    })
  );
  // downsize the original layer
  model.add(
  ...
  model.add(
    tf.layers.dense({
      units: 10,
      kernelInitializer: "VarianceScaling",
      activation: "softmax" // use to predict which one is the highest possibility in 10 units
    })
  );

  createLogEntry("Layer Created...");
...
async function predict(batch) {
  tf.tidy(() => {
    ...
    if (prediction_value - input_value == 0) {
      ...
      label.innerHTML += "<br>Recognized failed!!";
    }
...
document
  ...
  .addEventListener("click", async (element, event) => {
